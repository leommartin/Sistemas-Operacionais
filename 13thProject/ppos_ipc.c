...
{   
    ...
    #ifdef DEBUG
        printf("\t--DEBUG: sem_init: semaphore initialized with success\n\n");
    #endif
    return 0;
}
...
int sem_down (semaphore_t *s) 
{
    #ifdef DEBUG
        printf("\n\t------------- SEMAPHORE ------------- \n");
        printf("\t--DEBUG: sem_down(): semaphore requested.\n\n");
    #endif
    if (s == NULL)
    {
        return -1;
    ...
    {
        #ifdef DEBUG
            printf("\t--DEBUG: sem_down(): task suspended and go to semaphore queue.\n");
...
int sem_up (semaphore_t *s) 
{
     #ifdef DEBUG
        printf("\n\t------------- SEMAPHORE ------------- \n");
        printf("\t--DEBUG: sem_up: semaphore up.\n\n");
    #endif
    ...
    if (s->counter <= 0)
    {
        ...
        // #ifdef DEBUG
        //     printf("\t--DEBUG: sem_down: task %d reactivated and go to READY tasks queue.\n", current_task->id);
        //     print_task_queue((queue_t*)s->queue);
...
int sem_destroy (semaphore_t *s) 
{
    ...
    {
        // #ifdef DEBUG
        //     printf("\t--DEBUG: sem_destroy(): task %d removed of the semaphore queue and added to READY taks queue.\n", s->queue->id);
        // // #endif
        task_awake(s->queue, &s->queue);
    } 
